...
#The model did great, it repeatedly partitioning the data into multiple sub-spaces, so that the outcomes in each final sub-space is as homogeneous as possible. 
#We also got a  binary tree visualization.
#It represented a tree model that predicted  the species of iris flower based on the length (in cm) and width of sepal and petal.
#The plot showed the different possible splitting rules that can be used to effectively predict the type of outcome (here, iris species). 
#For example, the top split assigns observations having Petal.length < 2.45 to the left branch, where the predicted species are setosa.

]#TASK: What percent of cases were correctly classified? 0.978
